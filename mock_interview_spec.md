...
- Job Planner 뷰: `backend/core/views/job_planner_view.py`
...
`backend/core/views/job_planner_view.py`의 `JobPlannerParseView.post()` 수정:
...
frontend/src/features/interview/
  ...
  components/
    ...
    TurnFeedback.vue         # 슬롯 상태 진행도 표시
  composables/
    useInterview.js          # 면접 세션 상태 관리
    useSSE.js                # SSE 스트리밍 처리
  api/
...
# PHASE 2: 음성/화상 전환 (면접 엔진 완성 후)

Phase 1의 면접 엔진 코드는 변경하지 않는다.
음성 모듈은 면접 엔진 앞단에 독립적으로 붙는다.
...
웹캠 → 화면 표시 (분석하지 않음, 면접 분위기 조성용)
```

## Phase 2에서 추가/수정하는 파일
...
backend/core/services/
  stt/
    __init__.py
    vad.py              # silero-vad 기반 음성 활동 감지
    transcriber.py      # faster-whisper large-v3 음성→텍스트 변환
    stt_service.py      # VAD + STT 통합 파이프라인

backend/core/views/interview/
  stt_view.py           # STT API 엔드포인트 (interview 폴더 안에 추가)
```
...
STT: faster-whisper large-v3
  - CTranslate2 기반 Whisper 최적화 버전
  - 로컬 실행 (네트워크 지연 없음, 비용 없음)
...
```
변경 파일:
  frontend/src/features/interview/MockInterview.vue
    → 텍스트 입력 영역을 음성 녹음 컴포넌트로 교체

추가 파일:
  frontend/src/features/interview/components/
    AudioRecorder.vue    # 마이크 녹음 + VAD 연동
    WebcamDisplay.vue    # 웹캠 화면 표시 (분석 없음)
```
...
1. 사용자가 "답변 시작" 버튼 클릭
2. 마이크 + 웹캠 활성화
3. 음성 녹음 시작 (MediaRecorder)
4. 침묵 감지 또는 "답변 완료" 버튼으로 녹음 종료
5. 음성 파일 → /api/core/stt/transcribe/ 전송
6. 텍스트 수신 → 사용자에게 "이렇게 인식했습니다" 확인 표시
7. 확인 후 → /api/core/interview/sessions/<id>/answer/ 전달 (기존 흐름과 동일)
```

### Phase 2에서 면접 엔진(Phase 1) 수정 사항

**없음.**

`answer_view.py`의 answer 엔드포인트는 `{"answer": "텍스트"}`를 받는다.
이 텍스트가 키보드에서 왔든 STT에서 왔든 면접 엔진은 동일하게 처리한다.
