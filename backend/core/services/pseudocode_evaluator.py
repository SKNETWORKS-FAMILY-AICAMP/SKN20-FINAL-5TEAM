...
class LLMEvaluationEngine:
    ...
    def _build_prompts(
        ...
        - **채점 원칙**: 아래 [3]의 5차원 루브릭을 기준으로 각 차원을 독립적으로 채점한 뒤, 합산하여 overall_score를 결정하십시오. 루브릭 외의 최소 보장 점수는 없습니다.
        - 만점(85/85)은 도달 불가능한 점수가 아닙니다. 전문가 수준의 통찰력이 보인다면 과감하게 만점을 주십시오.
        - 문제에서 명시적으로 요구하지 않은 특정 라이브러리(예: GridSearchCV, Optuna 등)가 없다고 감점하지 마십시오.
        ...
        ▶ design (설계력) — {weights['design']}점
          [0-5]   논리 흐름 없음, 핵심 단계(데이터 분리/학습/예측)가 모두 누락
          [6-10]  1~2개 단계만 언급, 순서 불명확하거나 역방향
          [11-15] 핵심 흐름은 있으나 fit/transform 분리 등 구체성 부족
          [16-20] 분리 근거 명확, 전체 흐름 올바름. 일부 실무 패턴(Pipeline 등) 부재
          [21-25] Pipeline 구조화, 멱등성(Idempotency) 고려, 실무 즉시 적용 가능

        ▶ consistency (정합성) — {weights['consistency']}점
          [0-4]   train/test 혼용, 기준점 오염(Data Leakage) 발생
          [5-8]   분리 시도는 있으나 fit을 test에도 적용하는 등 모순 존재
          [9-12]  train 기준 fit, test에 transform만 적용. 일부 엣지에서 불일치
          [13-16] 데이터 분리 기준 일관성 유지, 직렬화 고려 시작
          [17-20] 직렬화 규칙 및 운영 환경에서의 추론 일관성 완벽 보장

        ▶ abstraction (추상화) — {weights['abstraction']}점
          [0-3]   단순 절차 나열, 하드코딩된 로직. 재사용 불가
          [4-6]   일부 변수명 추상화 시도, 함수 분리 없음
          [7-9]   함수/클래스 기반 모듈화 시도, 일부 하드코딩 잔존
          [10-12] 모듈화 완료, 파라미터화 시도. 다른 문제에도 일부 적용 가능
          [13-15] 다른 도메인에도 즉시 이식 가능한 설계 패턴 적용

        ▶ edgeCase (예외처리) — {weights['edgeCase']}점
          [0-3]   완성된 데이터만 상정한 Happy Path 설계. 예외 언급 전무
          [4-6]   결측치 또는 이상치 중 한 가지만 언급
          [7-9]   결측치/이상치 모두 언급, 처리 방식은 다소 추상적
          [10-12] 일반적인 데이터 이슈 대응 완료. 운영 장애 시나리오는 미흡
          [13-15] 드리프트, 리소스 부족, 분산 환경 장애 등 운영 시나리오까지 대응

        ▶ implementation (구현력) — {weights['implementation']}점
          [0-2]   자연어만으로 서술, 코드 힌트 전혀 없음
          [3-4]   자연어 위주이나 흐름은 이해됨 (최소한의 점수 부여)
          [5-6]   라이브러리(sklearn 등) 명칭 사용하나 파라미터 누락
          [7-8]   라이브러리 + 주요 파라미터 일부 명시
          [9-10]  구체적 파라미터, 메서드 체인, 로직 완전 명시
