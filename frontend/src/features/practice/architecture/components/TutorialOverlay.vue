...
const STEPS = [
  {
    selector: '.case-file-panel',
    title: 'MISSION BRIEFING',
    description: '여기서 시나리오, 미션, 제약조건을 확인하세요. 문제를 잘 읽고 설계를 시작합니다.',
    cardPosition: 'right'
  },
  {
    selector: '.toolbox-panel',
    title: 'COMPONENT PALETTE',
    description: '필요한 컴포넌트를 이곳에서 드래그하여 캔버스에 배치하세요.',
    cardPosition: 'right'
  },
  {
    selector: '.canvas-panel',
    title: 'ARCHITECTURE CANVAS',
    description: '컴포넌트를 배치하고 설계하는 공간입니다. 더블클릭으로 이름 변경, ✕ 버튼으로 삭제할 수 있습니다.',
    cardPosition: 'left'
  },
  {
    selector: '.header-controls > .ctrl-btn:nth-child(1)',
    title: 'CONNECT MODE',
    description: '이 버튼을 누르면 연결 모드로 전환됩니다. 연결 모드에서 컴포넌트 두 개를 순서대로 클릭하면 화살표로 연결됩니다.',
    cardPosition: 'bottom'
  },
  {
    selector: '.header-controls > .ctrl-btn.hint',
    title: 'HINT SYSTEM',
    description: '어떤 컴포넌트를 배치해야 할지 모르겠다면? 힌트를 켜면 필수 컴포넌트가 강조됩니다.',
    cardPosition: 'bottom'
  },
  {
    selector: '.header-controls > .ctrl-btn.danger',
    title: 'RESET',
    description: '캔버스를 초기화하고 처음부터 다시 설계할 수 있습니다.',
    cardPosition: 'bottom'
  },
  {
    selector: '.submit-btn',
    title: 'EXECUTE EVALUATION',
    description: '설계가 완료되면 이 버튼을 눌러 AI 평가를 시작하세요. 아키텍처 설명과 질문 답변 후 결과를 확인할 수 있습니다.',
    cardPosition: 'right'
  }
];
...
export default {
  ...
  methods: {
    ...
    handleOverlayClick() {
      // clicking on the dark overlay area advances to next step
      this.next();
    }
  }
};
