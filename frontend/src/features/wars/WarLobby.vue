<template>
  <div class="lobby-container">
    ...
    <footer class="lobby-footer">
      <div class="action-hint" v-if="onlineMembers.length < 2">
        최소 1명 이상의 팀원이 더 필요합니다. (테스트를 위해 혼자 시작할 수 있습니다)
      </div>
      <button 
...
const selectedRoleId = ref('architect');
const onlineMembers = ref([
  { id: 'me', name: '이민재', isMe: true, role: 'ARCHITECT' }
]);
...
const sendMsg = () => {
  ...
  sendTeamChat(gameStore.activeWarsMission.id, '이민재', newMsg.value);
  newMsg.value = '';
};
...
const selectRole = (roleId) => {
  ...
  onlineMembers.value[0].role = roleName;
  
  // 소켓을 통해 팀원들에게 역할 변경 알림
  if (gameStore.activeWarsMission) {
    changeRole(gameStore.activeWarsMission.id, roleName);
  }
};

const isRoleTaken = (roleId) => {
  // 현재는 1인 테스트 위주이므로 false, 추후 팀원 정보와 대조
  return false;
};

const startGame = () => {
  // 선택된 역할을 스토어에 저장하여 게임 룸에서 사용
  gameStore.setUserRole(selectedRoleId.value);
...
onMounted(() => {
  ...
  // 소켓 연결
  connectSocket(gameStore.activeWarsMission.id, '이민재');
});
